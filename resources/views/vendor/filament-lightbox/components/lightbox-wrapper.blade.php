...
<x-dynamic-component :component="$parentEntryWrapper" :entry="$entry">
    <a {{ $attributes->merge(
        ...
        class="glightbox" href="{{ $href }}" data-gallery="{{ $gallery }}" data-title="{{ $title }}"
        data-description="{{ $description }}" data-desc-position="{{ $descPosition }}" data-type="{{ $type }}"
        data-effect="{{ $effect }}" data-width="{{ $width }}" data-height="{{ $height }}"
        data-zoomable="{{ $zoomable }}" data-draggable="{{ $draggable }}" data-sizes="{{ $sizes }}"
        data-srcset="{{ $srtSet }}">
        {{ $slot }}
    </a>
</x-dynamic-component>
