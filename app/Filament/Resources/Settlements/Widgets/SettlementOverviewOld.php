...
class SettlementOverviewOld extends StatsOverviewWidget
{
    ...
    protected function getStats(): array
    {
        return [
            ...
            Stat::make('Nilai Pajak', '')
                ->view('volt-livewire::filament.widgets.custom-tax-widget',
                    [
                        'pph21' => RequestItem::where('tax_id', 1)->when($this->filter === 'this_month', fn (Builder $query) => $query->whereMonth('created_at', now(tz: 'Asia/Jakarta')->month))->get()->sum('tax_amount'),
                        'pph23' => RequestItem::where('tax_id', 2)->when($this->filter === 'this_month', fn (Builder $query) => $query->whereMonth('created_at', now(tz: 'Asia/Jakarta')->month))->get()->sum('tax_amount'),
                    ]),
